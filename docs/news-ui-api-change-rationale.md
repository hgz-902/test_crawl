...
- Requested behavior: 크롤링 서버에 UI 호출 API를 붙이고, 로컬 SQLite에서 기사 목록, 그룹 보기, 통계, 읽음/즐겨찾기, 감성분석, 분석 결과 저장/조회 흐름을 제공한다.
...
- Source change made:
  ...
  - `news_ui_api.py`: `/api/news`, `/api/news/grouped`, `/api/filter-options`, `/api/stats`, read/favorite, 분석 결과 save/get API 제공.
...
- Remaining risk:
  ...
  - 직접 LLM 호출용 `/api/news/{article_id}/analyze`는 사내 모델 사용 예정이어서 크롤링 서버에 포함하지 않았다.
