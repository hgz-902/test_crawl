...
# 스케줄러 command 결과 정보를 담는 데이터 객체다.
...
# 저장된 설정을 Windows Task Scheduler managed task와 동기화한다.
...
# windows schedule settings 유효성을 검증한다.
...
# managed task 목록을 조회해 목록으로 반환한다.
...
# 현재 프로젝트 namespace의 Windows Scheduler task 상세 상태를 조회한다.
...
# basic task details 값을 계산해 반환한다.
...
# powershell task details 값을 계산해 반환한다.
...
# 정지 스크립트를 호출해 현재 프로젝트의 managed task를 종료한다.
...
# ps quote 값을 계산해 반환한다.
def _ps_quote(value: str) -> str:
    return "'" + value.replace("'", "''") + "'"


# schedule args 값을 계산해 반환한다.
...
# schedule args job settings 값을 계산해 반환한다.
...
# schedule task plans job settings 값을 계산해 반환한다.
...
# cron schtasks task plans 값을 계산해 반환한다.
...
# cron schtasks args 값을 계산해 반환한다.
...
# split cron single schtasks expressions 값을 계산해 반환한다.
...
# finite cron values 값을 계산해 반환한다.
...
# cron multi trigger specs 값을 계산해 반환한다.
...
# multi trigger task xml를 생성해 반환한다.
...
# split task command 값을 계산해 반환한다.
...
# windows xml weekday 이름 값을 계산해 반환한다.
...
# multi trigger supported 값을 계산해 반환한다.
...
# multi trigger count 값을 계산해 반환한다.
...
# estimate invocations per hour 값을 계산해 반환한다.
...
# recommend cron strategy 값을 계산해 반환한다.
...
# windows command encoding 값을 계산해 반환한다.
...
# task not found message 여부를 판정한다.
