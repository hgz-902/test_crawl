...
def fetch_naver_news_api_items(api_url: str, timeout: float = 30.0) -> tuple[list[dict[str, Any]], str]:
    session = requests.Session()
    session.trust_env = False
    session.headers.update(_headers())
    response = session.get(api_url, timeout=timeout)
    response.raise_for_status()
    items = parse_naver_news_api_items(response.content, base_url=response.url, content_type=response.headers.get("Content-Type", ""))
