...
## Active Implementation Plan: Batch Job Orchestrator UI And Completion Email

### Status
- 2026-05-15: in progress through Alpha developer worker.

### Completion Criteria
- A web page lists existing crawler configs as batch jobs.
- The operator can enable/disable each config and set an interval in minutes.
- The operator can run a selected job immediately from the UI.
- The scheduler prevents concurrent runs for the same job and computes next run times.
- Completion email is addressed to `bloodknihts@gmail.com` and sent only when SMTP environment variables are configured.
- Job state and run history are persisted locally without storing SMTP secrets.
- Unit tests cover job persistence, route rendering, run-now behavior, and email skip behavior.
- Browser/UI QA confirms the orchestrator page loads and exposes the expected controls.

### Steps
1. Add a file-backed job store and run-history model.
2. Add env-gated completion email sender.
3. Add an in-app async scheduler that reuses `ConfigurableCrawler`.
4. Add `/jobs` UI and routes for save/toggle/run-now/status.
5. Add navigation from the existing layout.
6. Add regression tests and update `.env.example`/README.
7. Run Alpha reviewer, QA, and auditor checks.
8. Run unit tests and local UI/browser proof.
